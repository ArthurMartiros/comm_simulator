...
export class MediaServer {
  ...
  static RTP_PORT = 7078;
  static RTCP_PORT = 7079;
  ...
  public rtp: Udp.Socket;
  public rtcp: Udp.Socket;
  private client: any;
  ...
  constructor() {
    this.enabled = false;
    this.rtp = Udp.createSocket("udp4");
    this.rtcp = Udp.createSocket("udp4");
  ...
  public listen(host?: string, rtpPort?: number, rtcpPort?: number): Promise<any> {
    ...
    return detect.then((host: string) => {
      ...
      console.info(`LISTENING RTP:${rtpPort} / RTCP:${rtcpPort}`);
    });
  }
}