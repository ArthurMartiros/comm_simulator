...
export class RtcpPacket {
  static parse(buffer: Buffer) {
    switch (RtcpPacket.getType(buffer)) {
      case 200 :
        return new RtcpSRPacket(buffer);
      case 201 :
        return new RtcpRRPacket(buffer);
      case 202 :
        return new RtcpSDPacket(buffer);
      case 203 :
        return new RtcpGBPacket(buffer);
      case 204 :
        return new RtcpAPPacket(buffer);
    }
  }
  ...
  public copy() {
    var newBuffer = Buffer.alloc(this.buffer.length);
