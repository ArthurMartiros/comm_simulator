import { Call, CallDirection, CallState } from "./call";
...
export class IncomingInviteDialog extends InviteDialog {
  ...
  protected sendBye(req: Request) {
    ...
    MediaServer.instance.rtp.close();
    MediaServer.instance.rtcp.close();
    ...
    req.recordRoutes.forEach((r) => {
      if (!request.routes) {
        request.routes = [r];
      } else {
        request.routes.push(r);
      }
    });
  ...
  protected sendUpdateAccept(message: Request) {
    var response = new Response({
      ...
      content: Buffer.from(this.call.localSdp.toString())
    });
    this.emit('update');
    this.station.transport.send(response);
  }
  ...
  protected sendInviteAccept(message: Request) {
    var response = new Response({
      ...
      callId: message.callId,
      contentType: Mime.SDP,
      content: Buffer.from(MediaServer.listenTo(this.call).toString())
    });
    message.recordRoutes.forEach(r => {
      const rr = r.clone();
      //rr.uri.setParam('twnat', `sip:${message.to.uri.host}:${message.to.uri.port}`);
      if (!response.recordRoutes) {
        response.recordRoutes = [r];
      } else {
        response.recordRoutes.push(r);
      }
    });
    response.allow = ['INVITE', 'ACK', 'CANCEL', 'OPTIONS', 'BYE', 'REFER', 'NOTIFY', 'MESSAGE', 'SUBSCRIBE', 'INFO', 'UPDATE'];
    response.supported = ['replaces', 'outbound'];
    response.contact = message.to;
    response.contact.uri.setParam('transport', 'tcp');
    this.station.transport.send(response);
    this.emit('accept', this.call);
  }
