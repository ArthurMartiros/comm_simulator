...
import { Call, CallDirection } from "../dialogs/invitation/call";
import { RtpPacket }           from "../media/rtp";
...
export class Phone extends Station {
  ...
  constructor(username, server, transport: Transport) {
    ...
    this.oPack = new RtpPacket(Buffer.from(`
  ...
  onCall(call: Call) {
    ...
    call.on('ringing', () => {
      ...
      if (call.direction == CallDirection.INCOMING && call.to.uri.username == this.contact.uri.username) {
        console.info(`CALL ${cid} ${d} ${f} ${t} TAKING`);
        setTimeout(() => this.take(), 1000);
        setTimeout(() => this.drop(), 10000);
      }
    });
    call.on('update', () => {
